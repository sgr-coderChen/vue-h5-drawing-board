<template>
    <div id="draw-board" @click.stop="showColorPicker = true, showLineWidthRange = false">
        <ToolBox 
        ...
        <div v-show="showLineWidthRange" class="font-range">
            <input v-model="penAttibutes.width" class="font-range-input" type="range" >
        </div>
    </div>
</template>
...
export default {
    ...
    watch: {
        currentTool(newVal) {
            switch (newVal) {
                ...
                case 'eraser':
                    ...
                    // this.penAttibutes.strokeStyle = 'rgba(255, 255, 255, 1)'
                    // this.penAttibutes.fillStyle = '#000'
                    break;
                //  荧光笔（刷子）
                case 'brush':
                    let rgba = this.colorRgb(this.currentColor, 0.4)
                    this.penAttibutes.lineCap = "butt"
                    this.penAttibutes.strokeStyle = rgba
        ...
        currentColor(newVal) {
            console.log('currentColor', newVal)
            // const { r, g, b, a } = newVal.rgba
            // this.penAttibutes.strokeStyle = `rgba(${r}, ${g}, ${b}, ${a})`
            // this.penAttibutes.fillStyle = `rgba(${r}, ${g}, ${b}, ${a})`
    ...
    methods: {
        ...
        saveToHistory(){
            if (this.step === this.canvasHistory.length) {
                let nowImage = this.ctx.getImageData(0, 0, this.canvas.width, this.canvas.height)
